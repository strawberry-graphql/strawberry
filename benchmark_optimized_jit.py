"""Benchmark comparing standard GraphQL, JIT, and Optimized JIT compilers."""
...
def run_comprehensive_benchmark():
    ...
    for query_name, query in queries:
        ...
        for num_items in dataset_sizes:
            ...
            print(f"  Standard JIT:      {jit_time:.3f}s ({jit_speedup:.1f}x faster)")
            print(f"  Optimized JIT:     {opt_time:.3f}s ({opt_speedup:.1f}x faster)")
            ...
            # Verify correctness
            assert len(opt_result["contents"]) == len(standard_result.data["contents"])
            assert (
