...
class Query:
    ...
    def datasets(self, count: int = 5, points_per_dataset: int = 100) -> List[Dataset]:
        ...
        return [
            Dataset(name=f"Dataset-{i}", size=points_per_dataset)
...
def run_pure_computation_benchmark():
    ...
    total_computations = total_points * (fields_per_point + metrics_fields) + datasets * 6
    ...
    print(f"   Computations/sec (cached):    {(total_computations / (avg_cached/1000)):,.0f}")
    ...
    print(f"\n🚀 Bottom Line:")
    print(f"   Pure computation workloads show the TRUE power of JIT!")
    print(f"   • No I/O wait times to hide the optimization gains")
    print(f"   • Eliminates ALL GraphQL execution overhead")
    print(f"   • Near-native Python performance for field resolution")
