...
def demo_nullable_field_errors():
    ...
    print(f"  ✅ Successful fields: {sum(1 for p in result.data['store']['category']['products'] if p['name'])}")
    ...
    if "data" in jit_result:
        print(f"  ✅ Successful fields: {sum(1 for p in jit_result['data']['store']['category']['products'] if p['name'])}")
...
def demo_root_level_errors():
    ...
    print(f"  Data: {jit_result.get('data')}")
    if "errors" in jit_result:
        print(f"  ❌ Error: {jit_result['errors'][0]['message'] if jit_result['errors'] else 'None'}")
...
def demo_performance_with_errors():
    ...
    print(f"  Cached JIT:  {cached_time:.2f} ms ({standard_time/cached_time:.1f}x faster)")
