...
async def benchmark_query(query_name: str, query: str, iterations: int = 10):
    ...
    if times:
        ...
        print(
            f"JIT Cached:           {avg_cached * 1000:.2f}ms ({speedup:.2f}x faster)"
        )
...
async def simulate_production_load():
    ...
    print(
        f"  • Can handle {standard_time / jit_time:.1f}x more requests with same resources"
    )
