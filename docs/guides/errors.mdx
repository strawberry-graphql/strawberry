...
It is often best to express expected errors in the schema. This allows the
client to deal with the error in a robust way.
...
As seen in the [Unhandled execution errors][#unhandled-execution-errors] section
above, errors are automatically added to the errors array by raising the error
in execution.

But cases may arise where you want to handle errors or otherwise use the errors
array to indicate that issues have occurred, but still return data in the
response too.

This can be achieved by adding `Exception` instances to the `partial_errors`
...
Note that when exceptions are raised in resolvers, the GraphQL library catches
the exception and maps it to a `GraphQLError` with path and location data pulled
from the resolver. If you'd like to persist this kind of metadata in your
response, you'll need to add a `GraphQLError` with that data yourself, commonly
accomplished with the `located_error()` function.
