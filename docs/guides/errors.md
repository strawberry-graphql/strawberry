...
Handling expected errors is typically approached in one or both of two ways:

1. Return the error as part of the return type
2. Append the error to the `errors` array

### Returning as part of the return type

It is often best to express expected errors in the schema. This allows the client to deal with the error in a robust way.
...
But cases may arise where you want to handle errors or otherwise use the errors array to indicate that issues have occurred, but still return data in the response too.

This can be achieved by adding `Exception` instances to the `partial_errors` array on the context.
...
def query(self, info) -> bool:
    ...
    info.context.partial_errors.append(MyCustomException("Another partial failure"))
    return True
```
...
Note that when exceptions are raised in resolvers, the GraphQL library catches the exception and maps it to a `GraphQLError` with path and location data pulled from the resolver.
If you'd like to persist this kind of metadata in your response, you'll need to add a `GraphQLError` with that data yourself.
