...
Strawberry provides a `Connection` class, which contains the basics of what you
need to implement the specification, but doesn't implement any pagination logic.
To use it, all you need to do is to subclass it and implement its abstract
`resolve_connection` classmethod.

<Note>

For basic cases, Strawberry provides a `ListConnection` which already implements
a `resolve_connection`, which we will look at in the next section.

</Note>

Let's look at an example for a connection of users:

```python
from collections.abc import Iterable

import strawberry
from strawberry.pagination import Connection, Edge, PageInfo, connection, to_base64
...
implements a limit/offset pagination algorithm by using slices.
...
resolved it will only generate as many results as needed for the given
pagination, the worst case scenario being the last results needing to be
returned.

## Max results

The implementation of `ListConnection` will limit the number of results to the
`connection_max_results` configuration in the
[schema's config](../../types/schema-configurations.md) (which defaults to
`100`).
...
@strawberry.type
class Query:
    @connection(ListConnection[User], max_results=10_000)
    def get_users(self) -> Iterable[User]:
        return some_function_that_returns_users()
```
...
class Query:
    @connection(ListConnection[User])
    def get_users(self, name_startswith: str) -> Iterable[User]:
...
class Query:
    @connection(UserConnection)
    def get_users(self, info: strawberry.Info) -> Iterable[UserModel]:
