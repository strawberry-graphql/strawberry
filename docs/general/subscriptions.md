...
supports multiple subscription transport protocols, each with different
trade-offs:
...
GraphQL Server Sent Events (`graphql-sse`), needs additional configuration (see
below).
...
- Authentication requires connection init params (headers not supported in
  browsers)
...
- SSE is HTTP/1.1 based and suffers from head-of-line blocking on HTTP/1.1
  ...
  blocking other resources (see
  [SSE Subscriptions](sse-subscriptions.md#http2-is-strongly-recommended-for-sse-subscriptions))
...
1. **Web/Mobile apps with complex real-time needs**: Use WebSocket with
