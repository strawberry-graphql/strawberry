...
schema = strawberry.federation.Schema(
    query=Query,
    enable_federation_2=True,  # Opt-in to Federation 2
)

# After (0.285.0+)
schema = strawberry.federation.Schema(
    query=Query,
    federation_version="2.11",  # Defaults to "2.11", always Federation 2
)
```
...
schema = strawberry.federation.Schema(
    query=Query,
    federation_version="2.5",  # Use a specific version
)
```
...
schema = strawberry.federation.Schema(
    query=Query,
    federation_version="2.5",  # Too old for @cost
)
```
...
schema = strawberry.federation.Schema(
    query=Query,
    enable_federation_2=True,  # Remove this line
)
```
