...
Like [object types](./object-types#extending-types), input types can be
assembled from more than one class by passing `extend=True`. This is useful when
a module needs to accept extra arguments on an input owned elsewhere:
...
@strawberry.input(name="SearchInput")
class SearchInput:
    query: str


@strawberry.input(name="SearchInput", extend=True)
...
class Query:
    @strawberry.field
    def search(self, input: SearchInput) -> list[str]:
        return []


schema = strawberry.Schema(query=Query, types=[SearchPagination])
```
...
`@strawberry.input(name: str = None, description: str = None, extend: bool = False)`
...
- `extend`: marks the class as extending an existing GraphQL input type. When
  another input with the same GraphQL name is present, this class's fields are
  merged into it and the type is printed as `extend input`.
