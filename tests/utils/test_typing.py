...
def test_get_optional_annotation():
    # Pair Union
    assert get_optional_annotation(str | bool | None) == str | bool

    # More than pair Union
    assert get_optional_annotation(str | int | bool | None) == str | int | bool
...
def test_eval_type():
    ...
    assert eval_type(ForwardRef("Optional[Foo]"), globals(), locals()) == Foo | None
    assert eval_type("Foo | None", globals(), locals()) == Foo | None
    assert eval_type(ForwardRef("Union[Foo, str]"), globals(), locals()) == Foo | str
    assert eval_type("Foo | str", globals(), locals()) == Foo | str
    assert (
