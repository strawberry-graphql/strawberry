...
def test_query_with_optional_fields():
    ...
    assert not result.errors
    assert result.data == {
        "getUser": {"name": "John", "email": "john@example.com", "age": None}
    }
...
def test_mutation_with_input_types():
    ...
    class Mutation:
        ...
        def create_user(self, input: CreateUserInput) -> User:
            return User(id=1, name=input.name, age=input.age, email=input.email)
...
def test_mutation_with_partial_input():
    ...
    class Mutation:
        ...
        def update_user(self, id: int, input: UpdateUserInput) -> User:
            ...
            return User(id=id, name=input.name or "Default Name", age=input.age or 18)
    ...
    assert not result.errors
    assert result.data == {"updateUser": {"id": 1, "name": "Updated Name", "age": 18}}
...
def test_complex_pydantic_types_execution():
    ...
    assert result.data == {
        "getUser": {
            ...
            "isActive": True,
            "tags": ["developer", "python", "graphql"],
            "profile": {"bio": "Software developer", "website": "https://johndoe.com"},
        }
    }
...
def test_pydantic_interface_basic():
    ...
    assert not result.errors
    assert result.data == {"getUser": {"id": "user_1", "name": "John"}}
...
def test_strawberry_private_fields_not_in_schema():
    ...
    assert not result.errors
    assert result.data == {"getUser": {"id": 1, "name": "John"}}
