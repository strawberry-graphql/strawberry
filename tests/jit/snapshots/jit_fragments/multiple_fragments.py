...
Query Details:
- Operation: query GetPostsMultipleFragments
- Root Type: Query
    ...
    if field_posts_value is not None:
        if isinstance(field_posts_value, list):
            ...
            for item_posts in field_posts_value:
                ...
                field_content_value = resolver(item_posts, info)
                item_posts_result["content"] = field_content_value
                ...
                if field_author_value is not None:
                    ...
                    field_bio_value = resolver(field_author_value, info)
            ...
            field_content_value = resolver(field_posts_value, info)
            single_item_result["content"] = field_content_value
            info.field_name = "published"
            info.parent_type = "Post"
            ...
            if field_author_value is not None:
                ...
                info.field_name = "name"
                info.parent_type = "Author"
                resolver = _resolvers['resolver_31']
                field_name_value = resolver(field_author_value, info)
                nested_author_result["name"] = field_name_value
                info.field_name = "bio"
                info.parent_type = "Author"
                resolver = _resolvers['resolver_32']
                field_bio_value = resolver(field_author_value, info)
                nested_author_result["bio"] = field_bio_value
                single_item_result["author"] = nested_author_result
            else:
...
_resolvers['resolver_31'] = _default_resolver  # Custom resolver in actual execution
_resolvers['resolver_32'] = _default_resolver  # Custom resolver in actual execution
