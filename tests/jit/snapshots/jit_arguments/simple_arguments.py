...
def execute_query(root, context=None, variables=None):
    ...
    info.field_name = "user"
    info.parent_type = "Query"
    kwargs = {}
    kwargs['id'] = '123'
    resolver = _resolvers['resolver_0']
    field_user_value = resolver(root, info, **kwargs)
    if field_user_value is not None:
        nested_user_result = {}
        info.field_name = "id"
        info.parent_type = "User"
        resolver = _resolvers['resolver_1']
        field_id_value = resolver(field_user_value, info)
        nested_user_result["id"] = field_id_value
        info.field_name = "name"
        info.parent_type = "User"
        resolver = _resolvers['resolver_2']
        field_name_value = resolver(field_user_value, info)
        nested_user_result["name"] = field_name_value
        info.field_name = "email"
        info.parent_type = "User"
        resolver = _resolvers['resolver_3']
        field_email_value = resolver(field_user_value, info)
        nested_user_result["email"] = field_email_value
        ...
        field_posts_value = resolver(field_user_value, info, **kwargs)
        if field_posts_value is not None:
            if isinstance(field_posts_value, list):
                nested_user_result["posts"] = []
                for item_posts in field_posts_value:
                    ...
                    field_published_value = resolver(item_posts, info)
                    item_posts_result["published"] = field_published_value
                    nested_user_result["posts"].append(item_posts_result)
            else:
                single_item_result = {}
                info.field_name = "id"
                info.parent_type = "Post"
                ...
                field_published_value = resolver(field_posts_value, info)
                single_item_result["published"] = field_published_value
                nested_user_result["posts"] = single_item_result
        else:
            nested_user_result["posts"] = None
        result["user"] = nested_user_result
    else:
        result["user"] = None
    return result
