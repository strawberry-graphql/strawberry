        ...
        if isinstance(field_posts_value, list):
            ...
            for item_posts in field_posts_value:
                ...
                if field_author_value is not None:
                    ...
                    nested_author_result["fullName"] = field_name_value
                    item_posts_result["writer"] = nested_author_result
                else:
                    item_posts_result["writer"] = None
                result["allPosts"].append(item_posts_result)
        else:
            single_item_result = {}
            info.field_name = "id"
            info.parent_type = "Post"
            ...
            single_item_result["postId"] = field_id_value
            info.field_name = "title"
            info.parent_type = "Post"
            resolver = _resolvers['resolver_6']
            field_title_value = resolver(field_posts_value, info)
            single_item_result["headline"] = field_title_value
            info.field_name = "author"
            info.parent_type = "Post"
            ...
            if field_author_value is not None:
                ...
                field_name_value = resolver(field_author_value, info)
                nested_author_result["fullName"] = field_name_value
                single_item_result["writer"] = nested_author_result
            else:
                single_item_result["writer"] = None
            result["allPosts"] = single_item_result
    else:
        result["allPosts"] = None
    return result
