...
Query Details:
- Operation: query MixedQuery
- Root Type: Query
- Top-level Fields: syncPosts, posts
- Generated: [timestamp]
"""
...
async def execute_query(root, context=None, variables=None):
    ...
    info.field_name = "syncPosts"
    info.parent_type = "Query"
    resolver = _resolvers['resolver_0']
    field_syncPosts_value = resolver(root, info)
    if field_syncPosts_value is not None:
        if isinstance(field_syncPosts_value, list):
            result["syncPosts"] = []
            for item_syncPosts in field_syncPosts_value:
                item_syncPosts_result = {}
                info.field_name = "id"
                info.parent_type = "Post"
                resolver = _resolvers['resolver_1']
                field_id_value = resolver(item_syncPosts, info)
                item_syncPosts_result["id"] = field_id_value
                info.field_name = "title"
                info.parent_type = "Post"
                resolver = _resolvers['resolver_2']
                field_title_value = resolver(item_syncPosts, info)
                item_syncPosts_result["title"] = field_title_value
                info.field_name = "syncAuthor"
                info.parent_type = "Post"
                resolver = _resolvers['resolver_3']
                field_syncAuthor_value = resolver(item_syncPosts, info)
                if field_syncAuthor_value is not None:
                    nested_syncAuthor_result = {}
                    info.field_name = "name"
                    info.parent_type = "Author"
                    resolver = _resolvers['resolver_4']
                    field_name_value = resolver(field_syncAuthor_value, info)
                    nested_syncAuthor_result["name"] = field_name_value
                    item_syncPosts_result["syncAuthor"] = nested_syncAuthor_result
                else:
                    item_syncPosts_result["syncAuthor"] = None
                result["syncPosts"].append(item_syncPosts_result)
        else:
            single_item_result = {}
            info.field_name = "id"
            info.parent_type = "Post"
            resolver = _resolvers['resolver_5']
            field_id_value = resolver(field_syncPosts_value, info)
            single_item_result["id"] = field_id_value
            info.field_name = "title"
            info.parent_type = "Post"
            resolver = _resolvers['resolver_6']
            field_title_value = resolver(field_syncPosts_value, info)
            single_item_result["title"] = field_title_value
            info.field_name = "syncAuthor"
            info.parent_type = "Post"
            resolver = _resolvers['resolver_7']
            field_syncAuthor_value = resolver(field_syncPosts_value, info)
            if field_syncAuthor_value is not None:
                nested_syncAuthor_result = {}
                info.field_name = "name"
                info.parent_type = "Author"
                ...
                field_name_value = resolver(field_syncAuthor_value, info)
                nested_syncAuthor_result["name"] = field_name_value
                single_item_result["syncAuthor"] = nested_syncAuthor_result
            else:
                single_item_result["syncAuthor"] = None
            result["syncPosts"] = single_item_result
    else:
        result["syncPosts"] = None
