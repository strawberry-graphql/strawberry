...
from strawberry.jit_compiler_cached import CachedJITCompiler, compile_query_cached
...
def benchmark_compilation_overhead():
    ...
    print(f"Compilation overhead:     {avg_compilation/avg_execution:.1f}x execution time")
...
async def benchmark_cached_async_queries():
    ...
    print(f"  Cached:                {cached_time:.2f}s ({no_cache_time/cached_time:.2f}x faster)")
    print(f"  Cached + Parallel:     {cached_parallel_time:.2f}s ({no_cache_time/cached_parallel_time:.2f}x faster)")
...
def main():
    ...
    print("\n✅ KEY FINDINGS:")
    print(f"- Query compilation takes {comp_time/exec_time:.0f}x longer than execution")
    print(f"- Caching provides {prod_no_cache/prod_cache:.1f}x speedup in production scenarios")
    print(f"- Cache hit rates exceed {prod_stats.hit_rate:.0%} with realistic query distributions")
