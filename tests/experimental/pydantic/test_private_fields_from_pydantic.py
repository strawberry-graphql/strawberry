...
def test_all_fields_with_explicit_type_override():
    ...
    assert "age: String" in schema_str, (
        f"Expected 'age: String' in schema, got: {schema_str}"
    )
    assert "age: Int" not in schema_str, (
        f"Expected no 'age: Int' in schema, got: {schema_str}"
    )
